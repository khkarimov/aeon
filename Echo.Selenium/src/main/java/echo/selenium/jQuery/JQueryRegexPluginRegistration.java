    ...
    public String Prepare(String javaScript) {
        if (javaScript == null) {
			...
			/* if(!jQuery.expr[':'].regex) { 
			             ...
			             * }
			             */
        return javaScript.contains(":regex") ? ("if(!jQuery.expr[':'].regex)jQuery.expr[':'].regex=function(elem,index,match){var matchParams=match[3].split(','),validLabels=/^(data|css):/,attr={method:matchParams[0].match(validLabels)?matchParams[0].split(':')[0]:'attr',property:matchParams.shift().replace(validLabels,'')},regexFlags='ig',regex=new RegExp(matchParams.join('').replace(/^\\s+|\\s+$/g,''),regexFlags);return regex.test(jQuery(elem)[attr.method](attr.property))};" + Successor.Prepare(javaScript)) : Successor.Prepare(javaScript);
    }
}
