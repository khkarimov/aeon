...
public final class AppiumAdapterFactory extends SeleniumAdapterFactory {
    ...
    private IAdapter create(AppiumConfiguration configuration) {
        ...
        return new AppiumAdapter(driver, javaScriptFlowExecutor, asyncJavaScriptFlowExecutor, configuration, fallbackBrowserSize, seleniumHubUrl, loggingPreferences);
    }
        ...
        while (true) {
            try {
                ...
                if (currentTime < timeout) {
                    Sleep.getInstance().wait((int) configuration.getDouble(Configuration.Keys.THROTTLE, 100));
                    log.trace("Retrying");
    ...
    private void setContext() {
        if (StringUtils.isNotBlank(driverContext)) {
            log.trace("Switching to context {} as per configuration", driverContext);
